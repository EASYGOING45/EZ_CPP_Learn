...
- `vector<int>::const_iterator result = find(vec.begin(),vec.end(),seaarch_value);`
- 输入：两个标记范围的迭代器和目标查找值。
- 返回：如果找到，返回对应的迭代器，否则返回第二个参数，即标记结尾的迭代器

## 初识泛型算法

- 标准库提供了超过100个算啊发啊，但这些算法有着一致的结构
- 理解算法的最基本的方法是了解它们是否读取元素、改变元素、重排元素顺序

### 只读算法

- 只读取范围中的元素，不改变元素
- 如 `find`和 `accumulate`（在numeric中定义，用于求和）
- `find_first_of`，
  - 输入：两队迭代器标记两段范围，在第一段中找第二段中任意元素，返回第一个匹配的元素，
  - 找不到则返回第一段的 `end`迭代器
...
- 插入器是一种迭代器适配器，接受一个容器，生成一个迭代器，能实现向给定容器添加元素的操作
- 三种类型：
