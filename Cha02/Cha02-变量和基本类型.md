...
- 左值（l-value） 可以出现在赋值语句的左边或右边，比如变量
...
- 引用必须初始化
- 引用和它的初始值是绑定在一起的，而不是拷贝，一旦定义就不能再被更改绑定为其他的对象
...
- 指针的值的四种状态：
  ...
  - > 对无效指针的操作均会引发错误，第二种和第三种虽然有效，但理论上不被允许
...
- const对象必须初始化，且不能被改变
- const变量默认不能被其他文件所访问，必须要访问的话，需要在指定const定义之前加extern。
- 要想在多个文件中使用const变量共享，定义和声明都加const关键字即可。
